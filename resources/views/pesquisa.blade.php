...
@section('content')
	...
	<center>	
		<nav aria-label="Page navigation example">
		  <ul class="pagination">
		    <li class="page-item">
		      <a class="page-link" href="{{ route('pesquisar')}}/{{$ant}}" aria-label="Voltar">
		    ...
		    @for($i = 1; $i <= ceil(count($paginar) / 5); $i++)
			    ...
			    <li class="page-item">
			    	<a class="page-link" href="{{ route('pesquisar')}}/{{$i}}">{{$i}}</a>
			    </li>

		    @endfor

		    <li class="page-item">
		      <a class="page-link" href="{{ route('pesquisar')}}/{{$nx}}" aria-label="Proximo">
